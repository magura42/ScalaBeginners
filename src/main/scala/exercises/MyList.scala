...
// Exercise to OO. Abschnitt 3, Lektion 20

// 3,20: 1. Generic trait MyPredicate[-T] with a little method test(T) => Boolean
trait MyPredicate[-T] {
  def test(eleem: T): Boolean
}

// 3,20: 2. Generic trait MyTransformer[-A, B] with a method transform(A) => B
